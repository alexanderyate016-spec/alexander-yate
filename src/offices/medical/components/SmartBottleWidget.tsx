...
import { Droplet, Plus, Trash2, CheckCircle2, Sparkles, Settings } from 'lucide-react';
...
export const SmartBottleWidget: React.FC<Props> = ({ data, todayStr }) => {
  ...
  const handleAddWater = (amountMl: number) => {
    MedicalStore.addWaterIntake(todayStr, amountMl);
  };
  ...
  const customBottles = data.customBottles || [
    { id: 'def_1', name: 'Vaso estándar', capacityMl: 250 },
    { id: 'def_2', name: 'Botella Gym', capacityMl: 600 },
    { id: 'def_3', name: 'Termo Oficina', capacityMl: 750 },
    { id: 'def_4', name: 'Botella Gran Formato', capacityMl: 1000 }
  ];
  ...
  return (
    <GlassPanel accentColor="blue" padding="lg" className="space-y-6 bg-gradient-to-br from-[#061B2E]/90 to-[#0B2A4A]/80 border-cyan-500/30">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-cyan-200/80 font-sans">
              Seguimiento preciso de consumo de agua con llenado visual interactivo
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
        ...
        <div className="md:col-span-5 flex flex-col items-center justify-center p-4 bg-cyan-950/30 border border-cyan-500/20 rounded-2xl relative overflow-hidden group">
          {/* Ambient Glow */}
          ...
          <div className="relative w-36 h-64 flex items-center justify-center">
            <svg viewBox="0 0 100 200" className="w-full h-full drop-shadow-[0_0_15px_rgba(6,182,212,0.3)]">
              <defs>
          ...
          <div className="mt-2 text-center">
            <p className="text-xs text-slate-300 font-medium">Progreso de hoy</p>
            <p className="text-sm font-bold text-cyan-200 font-mono">
        ...
        <div className="md:col-span-7 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3.5 bg-cyan-950/40 border border-cyan-500/30 rounded-xl space-y-1">
              <span className="text-[11px] uppercase tracking-wider text-cyan-300 font-bold block">Has tomado</span>
              ...
              <p className="text-[11px] text-cyan-300/80 font-mono font-medium">{fillPct}% del objetivo</p>
            </div>
            ...
            <div className="p-3.5 bg-slate-900/60 border border-slate-700/60 rounded-xl space-y-1">
              ...
              <p className="text-[11px] text-slate-400 font-mono">
                {remainingMl > 0 ? `~${Math.ceil(remainingMl / 250)} vasos restantes` : '¡Completado! 🎉'}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <label className="text-xs font-bold uppercase tracking-wider text-cyan-200 flex items-center gap-1.5">
                ...
                Añadir al instante (Tus Botellas y Vasos)
              </label>
              <button
              ...
              >
                <Plus className="w-3 h-3" /> Crear Botella
              </button>
            </div>
            ...
            {showAddBottle && (
              <form onSubmit={handleCreateBottle} className="p-3 bg-[#081C30] border border-cyan-500/40 rounded-xl space-y-2 animate-fadeIn">
                <div className="grid grid-cols-2 gap-2">
                  <input
                    type="text"
                    placeholder="Ej: Termo Gym / Vaso Agua"
                    value={newBottleName}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {customBottles.map(bot => (
                <button
                  key={bot.id}
                  onClick={() => handleAddWater(bot.capacityMl)}
          ...
          <div className="pt-2 border-t border-cyan-500/20 flex items-center gap-2">
            <input
              type="number"
              placeholder="Registrar cantidad exacta en ml..."
              value={manualMl}
              onChange={e => setManualMl(e.target.value === '' ? '' : Number(e.target.value))}
              className="flex-1 p-2 bg-[#081C30] border border-cyan-500/30 rounded-xl text-xs text-cyan-100 placeholder-slate-400 focus:outline-none focus:border-cyan-400"
            />
            <button
              onClick={() => {
                if (manualMl && Number(manualMl) > 0) {
                  handleAddWater(Number(manualMl));
                  setManualMl('');
                }
              }}
              className="px-4 py-2 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold text-xs rounded-xl transition-all shadow-md active:scale-95 flex items-center gap-1.5 shrink-0"
            >
